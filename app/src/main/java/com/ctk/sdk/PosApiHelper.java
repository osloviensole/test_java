...
/*--------------------------------    printer      --------------------------------------------*/
     ...
     * @return
     */
    synchronized public  int PrintSetFontTTF( String filename,byte FontWeight, byte FontHeight){
        if (mPosService != null) {
            try {
                return mPosService.Lib_SetFontTTF(filename, FontWeight,  FontHeight);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
     ...
     * @return
     */
    synchronized public  int PrintTableText( String[] texts, int[] widths, int[] align){
        if (mPosService != null) {
            try {
                return mPosService.Lib_PrintTableText(texts,  widths,  align);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * 修改字体打印TTF时候的打印方向，0为从左到右，1为从右到左
     * @param
     * @return
     */
    synchronized public  int PrintSetDirection(  byte direction){
        if (mPosService != null) {
            try {
                return mPosService.Lib_SetDirection(direction);
            } catch (RemoteException e) {
                Log.e( "s","FONTTTFWRONG");
                e.printStackTrace();
            }
        }
        return 0x11;
    ...
    /**
     * @Title: PrintSetItalic
     * @Description: set italic
     * @param:
     *       mode:
     *       0(default) -> normal
     *       1          -> italic font
     ...
     */
    //4.0.26
    synchronized public int PrintSetItalic(int mode){
        if (mPosService != null) {
            try {
                return mPosService.Lib_PrintSetItalic(mode);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
